...
#include "db.h"
#include "shell.h"
int Command_clean(apr_pool_t *p)
{
    int rc;
    rc=Shell_exec(CLEANUP_SH,NULL);
    check(rc==0,"Clean dir Failed!");
    return 0;
error:
    return -1;
}
...
int Command_fetch(apr_pool_t *p,const char *url,int fetch_only)
{
	...
	rc=Shell_exec(TAR_SH,"TAR_OPT","-xzf","FILE",TAR_GZ_SRC,NULL);
	...
	rc=Shell_exec(TAR_SH,"TAR_OPT","-xjf","FILE",TAR_BZ2_SRC,NULL);
		    ...
		    const char *make_opts,const char *install_opts)
{
    ...
    printf("\n\n\n The Return Value of Command_fetch is %d\n",rc);
